    ...
    if scapy_packet.haslayer(scapy.Raw):
        if scapy_packet.haslayer(scapy.TCP):
            if scapy_packet[scapy.TCP].dport == 80:
                ...
                if replace_load:
                    modified_load = modify_load(replace_load, scapy_packet[scapy.Raw].load)
                ...
                #print(load_res)
                content_length = re.search("(?:Content-Length:\s)(\d*)", load_res)
                if content_length and b"text/html" in modified_load:
                    ...
                    #print(new_len)
                    modified_load = modified_load.replace(content_length, new_len)
                    #print(modified_load)
                temp_packet = set_load(scapy_packet, modified_load)
